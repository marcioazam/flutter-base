...
void main() {
  group('Contrast Ratio Properties', () {
    test('Black on white meets WCAG AA', () {
      final ratio = AccessibilityUtils.contrastRatio(
        Colors.black,
        Colors.white,
      );
      expect(
        ratio,
        greaterThanOrEqualTo(AccessibilityUtils.minContrastRatioAA),
      );
      expect(
    ...
    test('White on black meets WCAG AA', () {
      ...
      expect(
        AccessibilityUtils.meetsContrastAA(Colors.white, Colors.black),
        isTrue,
      );
    });
    ...
    Glados<int>().test('Contrast ratio is always between 1 and 21', (seed) {
      final r = seed.abs() % 256;
      final g = ((seed * 7) % 256).abs();
      final b = ((seed * 13) % 256).abs();
      final color = Color.fromARGB(255, r, g, b);

      final ratioWithWhite = AccessibilityUtils.contrastRatio(
        color,
        Colors.white,
      );
      final ratioWithBlack = AccessibilityUtils.contrastRatio(
        color,
        Colors.black,
      );
    ...
    Glados<int>().test('ensureContrast returns color meeting minimum ratio', (
      seed,
    ) {
      final r = seed.abs() % 256;
      final g = ((seed * 7) % 256).abs();
      ...
      final ensured = AccessibilityUtils.ensureContrast(foreground, background);
  ...
  group('Relative Luminance Properties', () {
    ...
    Glados<int>().test('Luminance is always between 0 and 1', (seed) {
      final r = seed.abs() % 256;
      final g = ((seed * 7) % 256).abs();
      final b = ((seed * 13) % 256).abs();
      final color = Color.fromARGB(255, r, g, b);
  ...
  group('Touch Target Properties', () {
    test('48x48 is valid touch target', () {
      expect(AccessibilityUtils.isValidTouchTarget(const Size(48, 48)), isTrue);
    });
    ...
    test('ensureTouchTargetSize returns at least 48x48', () {
      final small = AccessibilityUtils.ensureTouchTargetSize(
        const Size(20, 20),
      );
      expect(small.width, equals(48.0));
      expect(small.height, equals(48.0));

      final large = AccessibilityUtils.ensureTouchTargetSize(
        const Size(100, 100),
      );
    ...
    Glados<int>().test('ensureTouchTargetSize always returns valid size', (
      seed,
    ) {
  ...
  group('Semantic Colors', () {
    test('Light semantic colors have sufficient contrast', () {
      ...
      expect(
        AccessibilityUtils.meetsContrastAA(colors.errorOnSurface, colors.error),
        isTrue,
      );
      expect(
        AccessibilityUtils.meetsContrastAA(colors.infoOnSurface, colors.info),
        isTrue,
      );
    });
